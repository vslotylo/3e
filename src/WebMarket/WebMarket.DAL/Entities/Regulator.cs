...
namespace WebMarket.DAL.Entities
{
    public class Regulator : ElectricProduct
    {
        ...
        protected override void InitializeProductInfos()
        {
            ...
            if (PowerCapacity > 0)
            {
                this.Infos.Add(new ProductInfo { Name = "Потужність", Value = string.Format("{0} {1}", this.PowerCapacity, "кВт"), IsPreview = true });
            }
            if (PhaseCount > 0)
            {
                this.Infos.Add(new ProductInfo { Name = "Кількість фаз", Value = string.Format("{0}", this.PhaseCount), IsPreview = true });
            }
        }
    }
}
